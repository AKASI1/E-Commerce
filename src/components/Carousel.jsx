...
export default function Carousel({ slides }) {
  ...
  const prevSlide = () =>
    setSlide(currentSlide === 0 ? slides.length - 1 : currentSlide - 1);
          ...
          >
            ...
            <div
              className={clsx(
                ...
                "sm:(items-start text-right)"
              )}
            ></div>
            <div
              className={clsx(
                "absolute right-0 top-0 px-10",
                "max-w-xl h-full flex flex-col justify-center items-center text-center",
                "text-white bg-gradient-to-r from-transparent to-gray-800/90",
                "sm:(items-start text-right)"
              )}
            >
              <h2 className="text-4xl sm:text-6xl font-bold mb-10">
                اقوي كولكشن عبايات و اسدالات
              </h2>
              <p className="text-xl mb-10">
                تشكيلة ملابس منزلية مميزة للنساء اكتشفيها الآن!
              </p>
